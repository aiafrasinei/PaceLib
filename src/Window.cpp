...
Window::Window(Configuration *conf)
{
    success = true;
		...
		ConLog::Error("SDL could not initialize! SDL Error: " + string(SDL_GetError()));
		success = false;
	} else {
		...
		if(conf->Get("Fullscreen") == 0) { 
			window = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, conf->Get("Width"), conf->Get("Height"),  SDL_WINDOW_SHOWN);
		} else if (conf->Get("Fullscreen") == 1) {
			window = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, conf->Get("Width"), conf->Get("Height"),  SDL_WINDOW_FULLSCREEN);
		...
		if(window == NULL) {
			...
			if(renderer == NULL) {
				...
				if(!(IMG_Init(img_flags) & img_flags)) {
					ConLog::Error("SDL_image could not initialize! SDL_image Error: " + string(IMG_GetError()));
					success = false;
				}

				if(TTF_Init() == -1) {
					ConLog::Error("SDL_ttf could not initialize! SDL_ttf Error: " + string(TTF_GetError()));
					success = false;
				}
			}
		}
