...
char *FC_GetStringASCII_Latin1(void) {
  if (ASCII_LATIN_1_STRING == nullptr)
    ASCII_LATIN_1_STRING =
        new_concat(FC_GetStringASCII(), FC_GetStringLatin1());

  return U8_strdup(ASCII_LATIN_1_STRING);
}

FC_Effect FC_MakeEffect(FC_AlignEnum alignment, FC_Scale scale,
                        SDL_Color color) {
  FC_Effect e;
...
FC_GlyphData FC_MakeGlyphData(int cache_level, Sint16 x, Sint16 y, Uint16 w,
                              Uint16 h) {
  FC_GlyphData gd;
...
static FC_GlyphData *FC_MapInsert(FC_Map *map, Uint32 codepoint,
                                  FC_GlyphData glyph) {
  Uint32 index;
  FC_MapNode *node;
  if (map == nullptr)
...
struct FC_Font {
  ...
  // Codepoints are little endian (reversed from UTF-8) so that something like
  // 0x00000005 is ASCII 5 and the map can be indexed by ASCII values
  FC_Map *glyphs;
...
// Private
static FC_GlyphData *FC_PackGlyphData(FC_Font *font, Uint32 codepoint,
                                      Uint16 width, Uint16 maxWidth,
                                      Uint16 maxHeight);

static SDL_Rect FC_RenderLeft(FC_Font *font, SDL_Renderer *dest, int x, int y,
                              FC_Scale scale, const char *text);
static SDL_Rect FC_RenderCenter(FC_Font *font, SDL_Renderer *dest, int x, int y,
                                FC_Scale scale, const char *text);
static SDL_Rect FC_RenderRight(FC_Font *font, SDL_Renderer *dest, int x, int y,
                               FC_Scale scale, const char *text);

static inline SDL_Surface *FC_CreateSurface32(Uint32 width, Uint32 height) {
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
  return SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0xFF000000,
                              0x00FF0000, 0x0000FF00, 0x000000FF);
#else
  return SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0x000000FF,
...
int U8_strinsert(char *string, int position, const char *source,
                 int max_bytes) {
  int pos_u8char;
  int len;
  int add_len;
...
SDL_Rect FC_DefaultRenderCallback(SDL_Texture *src, SDL_Rect *srcrect,
                                  SDL_Renderer *dest, int x, int y,
...
static SDL_Rect (*fc_render_callback)(SDL_Texture *src, SDL_Rect *srcrect,
                                      SDL_Renderer *dest, int x, int y,
                                      float xscale,
                                      float yscale) = &FC_DefaultRenderCallback;

void FC_SetRenderCallback(SDL_Rect (*callback)(SDL_Texture *src,
                                               SDL_Rect *srcrect,
                                               SDL_Renderer *dest, int x, int y,
                                               float xscale, float yscale)) {
  if (callback == nullptr)
...
unsigned int FC_GetBufferSize(void) { return fc_buffer_size; }
...
unsigned int FC_GetTabWidth(void) { return fc_tab_width; }
...
static void FC_Init(FC_Font *font) {
  ...
  // Give a little offset for when filtering/mipmaps are used.  Depending on
  // mipmap level, this will still not be enough.
  ...
  font->glyph_cache =
      (SDL_Texture **)malloc(font->glyph_cache_size * sizeof(SDL_Texture *));
...
static Uint8 FC_GrowGlyphCache(FC_Font *font) {
  ...
  SDL_Texture *new_level = SDL_CreateTexture(
      font->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
      font->height * 12, font->height * 12);

  if (new_level == nullptr ||
      !FC_SetGlyphCacheLevel(font, font->glyph_cache_count, new_level)) {
    SDL_Log("Error: SDL_FontCache ran out of packing space and could not add "
            "another cache level.\n");
  ...
  //   - for evading this bug, you must use FC_SetDefaultColor(), before using
  //   any draw functions
  set_color(new_level, font->default_color.r, font->default_color.g,
            font->default_color.b, FC_GET_ALPHA(font->default_color));
  {
    Uint8 r, g, b, a;
    ...
    // only backup if previous target existed (SDL will preserve them for the
    // default target)
    if (prev_target) {
...
Uint8 FC_UploadGlyphCache(FC_Font *font, int cache_level,
                          SDL_Surface *data_surface) {
  ...
  if (!fc_has_render_target_support)
    ...
    char old_filter_mode[16]; // Save it so we can change the hint value in the
                              // meantime
    ...
    new_level = SDL_CreateTexture(renderer, data_surface->format->format,
                                  SDL_TEXTUREACCESS_TARGET, data_surface->w,
                                  data_surface->h);
    SDL_SetTextureBlendMode(new_level, SDL_BLENDMODE_BLEND);
  ...
  if (new_level == nullptr ||
      !FC_SetGlyphCacheLevel(font, cache_level, new_level)) {
    SDL_Log("Error: SDL_FontCache ran out of packing space and could not add "
            "another cache level.\n");
    SDL_DestroyTexture(new_level);
...
static FC_GlyphData *FC_PackGlyphData(FC_Font *font, Uint32 codepoint,
  ...
  if (last_glyph->rect.x + last_glyph->rect.w + width >=
      maxWidth - FC_CACHE_PADDING) {
  ...
  return FC_MapInsert(glyphs, codepoint,
                      FC_MakeGlyphData(last_glyph->cache_level,
                                       last_glyph->rect.x, last_glyph->rect.y,
...
SDL_Texture *FC_GetGlyphCacheLevel(FC_Font *font, int cache_level) {
  if (font == nullptr || cache_level < 0 ||
      cache_level > font->glyph_cache_count)
    return nullptr;

  return font->glyph_cache[cache_level];
}

Uint8 FC_SetGlyphCacheLevel(FC_Font *font, int cache_level,
  ...
  if (cache_level == font->glyph_cache_count) {
    ...
    if (font->glyph_cache_count > font->glyph_cache_size) {
      ...
      new_cache = (SDL_Texture **)malloc(font->glyph_cache_count *
                                         sizeof(SDL_Texture *));
...
Uint8 FC_LoadFontFromTTF(FC_Font *font, SDL_Renderer *renderer, TTF_Font *ttf,
                         SDL_Color color) {
...

  {
    ...
    // Copy glyphs from the surface to the font texture and store the position
    // data Pack row by row into a square texture Try figuring out dimensions
    // that make sense for the font size.
    ...
    for (; *source_string != '\0'; source_string = U8_next(source_string)) {
      ...
      // Try packing.  If it fails, create a new surface for the next cache
      // level.
      packed = (FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0),
                                 glyph_surf->w, surfaces[num_surfaces - 1]->w,
                                 surfaces[num_surfaces - 1]->h) != nullptr);
      if (!packed) {
        int i = num_surfaces - 1;
        if (num_surfaces >= FC_LOAD_MAX_SURFACES) {
          // Can't do any more!
          SDL_Log("SDL_FontCache error: Could not create enough cache surfaces "
                  "to fit all of the loading string!\n");
          SDL_FreeSurface(glyph_surf);
          break;
        }

        // Upload the current surface to the glyph cache now so we can keep the
        // cache level packing cursor up to date as we go.
        FC_UploadGlyphCache(font, i, surfaces[i]);
        SDL_FreeSurface(surfaces[i]);

        SDL_SetTextureBlendMode(font->glyph_cache[i], SDL_BLENDMODE_BLEND);

        // Update the glyph cursor to the new cache level.  We need to do this
        // here because the actual cache lags behind our use of the packing
        // above.
      ...
      if (packed ||
          FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0),
                           glyph_surf->w, surfaces[num_surfaces - 1]->w,
                           surfaces[num_surfaces - 1]->h) != nullptr) {
        ...
        SDL_BlitSurface(glyph_surf, &srcRect, surfaces[num_surfaces - 1],
                        &destrect);
      }
...
Uint8 FC_LoadFont(FC_Font *font, SDL_Renderer *renderer,
                  const char *filename_ttf, Uint32 pointSize, SDL_Color color,
                  int style) {
  SDL_RWops *rwops;
...
Uint8 FC_LoadFont_RW(FC_Font *font, SDL_Renderer *renderer,
                     SDL_RWops *file_rwops_ttf, Uint8 own_rwops,
                     Uint32 pointSize, SDL_Color color, int style) {
  Uint8 result;
  TTF_Font *ttf;
  Uint8 outline;
...
void FC_FreeFont(FC_Font *font) {
  ...
  // If the last font has been freed; assume shutdown and free the global
  // variables
...
int FC_GetNumCacheLevels(FC_Font *font) { return font->glyph_cache_count; }
...
Uint8 FC_GetGlyphData(FC_Font *font, FC_GlyphData *result, Uint32 codepoint) {
  ...
  if (e == nullptr) {
    ...
    if (cache_image == nullptr) {
      SDL_Log("SDL_FontCache: Failed to load cache image, so cannot add new "
              "glyphs!\n");
      return 0;
    }
...
FC_GlyphData *FC_SetGlyphData(FC_Font *font, Uint32 codepoint,
...
static SDL_Rect FC_RenderLeft(FC_Font *font, SDL_Renderer *dest, int x, int y,
  ...
  for (; *c != '\0'; c++) {
    ...
    codepoint = FC_GetCodepointFromUTF8(
        &c, 1); // Increments 'c' to skip the extra UTF-8 bytes
    ...
    dstRect =
        fc_render_callback(FC_GetGlyphCacheLevel(font, glyph.cache_level),
                           &srcRect, dest, destX, destY, scale.x, scale.y);
...
SDL_Rect FC_Draw(FC_Font *font, SDL_Renderer *dest, int x, int y,
...
FC_StringList **FC_StringListPushBack(FC_StringList **node, char *value,
                                      Uint8 copy) {
  if (node == nullptr) {
    return nullptr;
  }
...
FC_StringList **FC_StringListPushBackBytes(FC_StringList **node,
...
static FC_StringList *FC_ExplodeBreakingSpace(const char *text,
                                              FC_StringList **spaces) {
  FC_StringList *head;
  FC_StringList **node;
  const char *start;
  const char *end;
...
static void FC_RenderAlign(FC_Font *font, SDL_Renderer *dest, int x, int y,
                           int width, FC_Scale scale, FC_AlignEnum align,
                           const char *text) {
  switch (align) {
  case FC_ALIGN_LEFT:
...
static FC_StringList *FC_GetBufferFitToColumn(FC_Font *font, int width,
                                              FC_Scale scale,
                                              Uint8 keep_newlines) {
  ...
  for (iter = ls; iter != nullptr; iter = iter->next) {
    ...
    if (width > 0 && FC_GetWidth(font, "%s", line) > width) {
      ...
      // Skip the first word for the iterator, so there will always be at least
      // one word per line
      line = new_concat(words->value, spaces->value);
      for (word_iter = words->next, spaces_iter = spaces->next;
           word_iter != nullptr && spaces_iter != nullptr;
           word_iter = word_iter->next, spaces_iter = spaces_iter->next) {
        char *line_plus_word = new_concat(line, word_iter->value);
        char *word_plus_space =
            new_concat(word_iter->value, spaces_iter->value);
...
static void FC_DrawColumnFromBuffer(FC_Font *font, SDL_Renderer *dest,
                                    SDL_Rect box, int *total_height,
                                    FC_Scale scale, FC_AlignEnum align) {
  int y = box.y;
  FC_StringList *ls, *iter;
...
SDL_Rect FC_DrawBox(FC_Font *font, SDL_Renderer *dest, SDL_Rect box,
                    const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == nullptr || font == nullptr)
...
SDL_Rect FC_DrawBoxAlign(FC_Font *font, SDL_Renderer *dest, SDL_Rect box,
                         FC_AlignEnum align, const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == nullptr || font == nullptr)
    return {box.x, box.y, 0, 0};
...
SDL_Rect FC_DrawBoxScale(FC_Font *font, SDL_Renderer *dest, SDL_Rect box,
                         FC_Scale scale, const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == nullptr || font == nullptr)
    return {box.x, box.y, 0, 0};
...
SDL_Rect FC_DrawBoxColor(FC_Font *font, SDL_Renderer *dest, SDL_Rect box,
                         SDL_Color color, const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == nullptr || font == nullptr)
    return {box.x, box.y, 0, 0};
...
SDL_Rect FC_DrawBoxEffect(FC_Font *font, SDL_Renderer *dest, SDL_Rect box,
                          FC_Effect effect, const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == nullptr || font == nullptr)
    return {box.x, box.y, 0, 0};
  ...
  FC_DrawColumnFromBuffer(font, dest, box, nullptr, effect.scale,
                          effect.alignment);
...
SDL_Rect FC_DrawColumn(FC_Font *font, SDL_Renderer *dest, int x, int y,
                       Uint16 width, const char *formatted_text, ...) {
  ...
  FC_DrawColumnFromBuffer(font, dest, box, &total_height, {1, 1},
                          FC_ALIGN_LEFT);

  return {box.x, box.y, width, total_height};
}

SDL_Rect FC_DrawColumnAlign(FC_Font *font, SDL_Renderer *dest, int x, int y,
                            Uint16 width, FC_AlignEnum align,
                            const char *formatted_text, ...) {
  SDL_Rect box = {x, y, width, 0};
  int total_height;
...
SDL_Rect FC_DrawColumnScale(FC_Font *font, SDL_Renderer *dest, int x, int y,
                            Uint16 width, FC_Scale scale,
                            const char *formatted_text, ...) {
  SDL_Rect box = {x, y, width, 0};
  int total_height;
...
SDL_Rect FC_DrawColumnColor(FC_Font *font, SDL_Renderer *dest, int x, int y,
                            Uint16 width, SDL_Color color,
                            const char *formatted_text, ...) {
  SDL_Rect box = {x, y, width, 0};
  int total_height;
...
SDL_Rect FC_DrawColumnEffect(FC_Font *font, SDL_Renderer *dest, int x, int y,
                             Uint16 width, FC_Effect effect,
                             const char *formatted_text, ...) {
  SDL_Rect box = {x, y, width, 0};
  int total_height;
  ...
  FC_DrawColumnFromBuffer(font, dest, box, &total_height, effect.scale,
                          effect.alignment);
...
static SDL_Rect FC_RenderCenter(FC_Font *font, SDL_Renderer *dest, int x, int y,
  ...
  for (c = str; *c != '\0';) {
    if (*c == '\n') {
      *c = '\0';
      result = SDL_RectUnion(
          FC_RenderLeft(font, dest,
                        x - scale.x * FC_GetWidth(font, "%s", str) / 2, y,
                        scale, str),
          result);
      *c = '\n';
      c++;
  ...
  result = SDL_RectUnion(
      FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str) / 2,
                    y, scale, str),
      result);
...
static SDL_Rect FC_RenderRight(FC_Font *font, SDL_Renderer *dest, int x, int y,
  ...
  for (c = str; *c != '\0';) {
    if (*c == '\n') {
      *c = '\0';
      result = SDL_RectUnion(
          FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str),
                        y, scale, str),
          result);
      *c = '\n';
      c++;
  ...
  result = SDL_RectUnion(
      FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str), y,
                    scale, str),
      result);
...
SDL_Rect FC_DrawScale(FC_Font *font, SDL_Renderer *dest, int x, int y,
                      FC_Scale scale, const char *formatted_text, ...) {
  if (formatted_text == nullptr || font == nullptr)
    return {x, y, 0, 0};
...
SDL_Rect FC_DrawAlign(FC_Font *font, SDL_Renderer *dest, int x, int y,
                      FC_AlignEnum align, const char *formatted_text, ...) {
  if (formatted_text == nullptr || font == nullptr)
    return {x, y, 0, 0};
...
SDL_Rect FC_DrawColor(FC_Font *font, SDL_Renderer *dest, int x, int y,
                      SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == nullptr || font == nullptr)
    return {x, y, 0, 0};
...
SDL_Rect FC_DrawEffect(FC_Font *font, SDL_Renderer *dest, int x, int y,
                       FC_Effect effect, const char *formatted_text, ...) {
  if (formatted_text == nullptr || font == nullptr)
    return {x, y, 0, 0};
...
Uint16 FC_GetHeight(FC_Font *font, const char *formatted_text, ...) {
  ...
  return font->height * numLines +
         font->lineSpacing * (numLines - 1); // height*numLines;
}
...
SDL_Rect FC_GetCharacterOffset(FC_Font *font, Uint16 position_index,
                               int column_width, const char *formatted_text,
                               ...) {
  ...
  if (formatted_text == nullptr || column_width == 0 || position_index == 0 ||
      font == nullptr)
    return result;
  ...
  for (iter = ls; iter != nullptr;) {
    ...
    ++num_lines;
    for (line = iter->value; line != nullptr && *line != '\0';
...
Uint16 FC_GetColumnHeight(FC_Font *font, Uint16 width,
                          const char *formatted_text, ...) {
  int y = 0;
...
SDL_Rect FC_GetBounds(FC_Font *font, int x, int y, FC_AlignEnum align,
                      FC_Scale scale, const char *formatted_text, ...) {
  SDL_Rect result = {x, y, 0, 0};
...
Uint8 FC_InRect(int x, int y, SDL_Rect input_rect) {
  return (input_rect.x <= x && x <= input_rect.x + input_rect.w &&
          input_rect.y <= y && y <= input_rect.y + input_rect.h);
}

// TODO: Make it work with alignment
Uint16 FC_GetPositionFromOffset(FC_Font *font, int x, int y, int column_width,
                                FC_AlignEnum align, const char *formatted_text,
                                ...) {
  FC_StringList *ls, *iter;
  Uint8 done = 0;
  ...
  for (iter = ls; iter != nullptr; iter = iter->next) {
    ...
    for (line = iter->value; line != nullptr && *line != '\0';
         line = (char *)U8_next(line)) {
      if (FC_GetGlyphData(font, &glyph_data,
                          FC_GetCodepointFromUTF8((const char **)&line, 0))) {
        if (FC_InRect(
                x, y,
                {current_x, current_y, glyph_data.rect.w, glyph_data.rect.h})) {
          done = 1;
          break;
        }
...
int FC_GetWrappedText(FC_Font *font, char *result, int max_result_size,
                      Uint16 width, const char *formatted_text, ...) {
  FC_StringList *ls, *iter;
